...
export default function CommitteeIssuesPage() {
  ...
  async function loadData() {
    try {
      ...
      setError(
        e.message || "Не вдалося завантажити дані.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function handleCreate(
    ...
    if (!form.title.trim() || !form.venueId) {
      setError(
        "Вкажіть назву та оберіть журнал або конференцію.",
      );
    ...
    try {
      ...
      setSuccess(
        "Випуск / збірник успішно створено.",
      );
  ...
  async function handleAddSubmission(
    ...
    const submissionId =
      selectedSubmissions[issueId];

    if (!submissionId) {
      setError(
        "Оберіть опубліковану статтю для додавання.",
      );
  ...
  async function handleRemoveSubmission(
    ...
    try {
      ...
      await removeSubmissionFromIssue(
        submissionId,
      );

      setSuccess(
        "Статтю прибрано з випуску.",
      );
  ...
  async function handlePublish(
    ...
    try {
      ...
      await publishPublicationIssue(
        issueId,
      );

      setSuccess(
        "Випуск успішно опубліковано.",
      );
      ...
      setError(
        e.message ||
          "Не вдалося опублікувати випуск.",
      );
    } finally {
      setActionLoadingId("");
    }
    ...
    <section className="committee-issues">
      <div className="committee-issues__container">
        <div className="committee-issues__hero">
          ...
          <h1>
            Формування випусків журналів
            та збірників конференцій
          </h1>

          <span>
            Створюйте випуски,
            додавайте опубліковані
            статті та публікуйте
            готові збірники.
          </span>
        </div>

        {error && (
          <div className="committee-issues__alert">
            {error}
          </div>
        )}
        ...
        <form
          className="committee-issues-form"
        ...
        <div className="committee-issues__list">
          {loading ? (
            ...
            issues.map((issue) => (
              <article
                key={issue.id}
                className="committee-issue-card"
              >
                ...
                <div className="committee-issue-card__submissions">
                  ...
                  {issue.submissions.length ===
                  0 ? (
                    <p>
                      Статті ще не додано.
                    </p>
                  ) : (
                    issue.submissions.map(
                      (submission) => (
                        <div
                          key={submission.id}
